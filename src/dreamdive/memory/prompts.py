    ...
    *,
    ...
    return PromptRequest(
        ...
        user=(
            ...
            f"Key values: {json.dumps(values)}\n"
            f"What they care most about: {json.dumps(top_concerns)}\n\n"
            f"EPISODIC BUFFER (entries older than {age_threshold} ticks):\n"
            f"{json.dumps(serializable_entries, indent=2, sort_keys=True)}\n\n"
            "PINNED ENTRIES (do not touch these):\n"
            f"{json.dumps(serializable_pinned, indent=2, sort_keys=True)}\n\n"
    ...
    *,
    ...
    return PromptRequest(
        ...
        user=(
            ...
            f"Unresolved threads: {json.dumps(current_arc_state.unresolved_threads)}\n"
            f"Ticks since last update: {ticks_elapsed}\n\n"
            "RECENT EVENTS:\n"
            f"{json.dumps(recent_event_log, indent=2, sort_keys=True)}\n\n"
            "ACTIVE AGENT STATES (compressed):\n"
            f"{json.dumps(agent_state_summary, indent=2, sort_keys=True)}\n\n"
