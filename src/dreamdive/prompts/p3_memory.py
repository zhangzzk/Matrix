...
from dreamdive.prompts.common import build_json_contract, meta_block as _meta_block
...
def build_memory_compression_prompt(
    *,
    ...
    meta_blk = _meta_block(meta_section)
    language_block = format_language_guidance_block(language_guidance)
    return PromptRequest(
        system=(
            "Compress old episodic memories for a character simulation. "
            "Preserve what matters, discard filler. Return valid JSON only."
        ),
        user=(
            f"{meta_blk}"
            f"{language_block}"
